	...
	<div style="float: left;border-radius: 5px;margin-top: 145px;">
		...
		<div >
			<table id="tbgw" style="margin-left: 90px;background-color:#E0FFFF;width: 980px;height: 60px;border-radius:25px;">
				<tbody>
				<tr><th style="text-align:center;"><h3>订单详情</h3></th></tr>
				</tbody>
			</table>
		</div>
		<div id="tdiv">
			<table  class=" table table-hover " style="width: 1000px;margin-left: 7%;background-color: #f2f2f2;padding: 20px;border-radius:25px;" id="th1">
				...
				<c:forEach items="${test}" var="i" varStatus="a" begin="0" end="${test.size()-1}">
					<tr><th>
						<div style="float: left;margin-top: 28px;height: 80px;">
                        <span style="float: left;margin-top: 10px;margin-right: 20px;">
                            <p style="border-radius: 100px;background: #8AC007;padding: 1px; width: 30px;height: 30px; padding-top:5px;">
					...
					<span style="float: right;">
							<button id="${i.value[0].id}" onclick="deleteoid(this)" style="margin-left: 30px;" class="btn-danger">✖</button>
...
<script type="text/javascript">
	...
	function index1() {
		document.location.href="/Order/gl/t3";
	}
	...
	function aaa(el) {
		var a1=$(el).attr("id");
		var a=$("#"+a1+"").attr("na");
		$.post("/Order/ds/signature/"+a+"",function(data){
			zhangshi(data);
		});
	};
	...
	//首页
	function azy() {
		$.post("/Order/ds/all",function(data){
			zhangshi1(data);
		});
	};
	...
	function suos() {
		...
		if(ssk==""){
			...
			$.post("/Order/ds/suos/"+ssk+"",function(data){
				zhangshi2(data);
			});
		}
	};
	...
	function zhangshi(data) {
		...
		for(var i=0;i<data.list.length;i++){
			html+="<thead><tr style="+"height:128px;"+">"+
					"<th class="+"col-md-2"+"><img id="+"img"+" src=/Order/img/"+data.list[i].dname+".jpeg style="+"width:110px;height:110px;"+"/></th>"+
	...
	function zhangshi1(data) {
		...
		for(var i=0;i<data.list.length;i++){
			html+="<tr style="+"height:128px;"+">"+
					"<th class="+"col-md-2"+" id="+"img"+data.list[i].id+"><img id="+"img"+" src=/Order/img/"+data.list[i].dname+".jpeg style="+"width:110px;height:110px;"+"/></th>"+
					"<th class="+"col-md-2"+" style="+"text-align:left;"+"><h2 id="+data.list[i].id+"dn>"+data.list[i].dname+"</h2></th>"+
					"<th class="+"col-md-2"+">"+
					"<form method="+"post"+">"+
					"<h3  style="+"margin-left:20px;"+" id="+"h3"+">"+
	...
	function xiugai1(el) {
		...
		var html2="";
		var html3="";
		$.post("/Order/vt/vsall",function(data){
	...
	function quxiao(el) {
		...
		var html2="";
		var html3="";
		$.post("/Order/vt/vsid/"+na+"",function(data){
		...
		html1+=dname;
		html2+=price+"元";
		html3+="<img id="+"img"+" src=/Order/img/"+dname+".jpeg style="+"width:110px;height:110px;"+"/>"
		$("#"+did+"dj").html(html);
		$("#"+did+"dn").html(html1);
		$("#"+did+"dp").html(html2);
		$("#img"+did+"").html(html3);
	...
	function shanchu(el) {
		...
		var dname=el.getAttribute("dname");
		if (confirm("确定删除")) {
			$.post("/Order/ds/deletedsid/"+id+"/"+dname+"",function(){
				alert("菜品删除成功");
				// azy();
				$.post("/Order/ds/all",function(data){
					zhangshi1(data);
	...
	function wancheng(el) {
		...
		$.post("/Order/gl/shanchuv/"+bid+"/"+value+"",function(data){
			leibie(data);
			alert("类别信息修改成功");
		})
	}
	...
	function dingdan(data) {
		...
		for(var i=0;i<data.length;i++){
			html+="<thead><tr style="+"height:128px;"+">"+
					"<th class="+"col-md-2"+">"+data[i].sid+"<img id="+"img"+" src=/Order/img/"+data[i].dname+".jpeg style="+"width:110px;height:110px;"+"/></th>"+
